var siteUrl2 = 'http://brandbugleindia.in/archana/';
 $.post(siteUrl2+'admin/adp/init_plugin',(data)=>{
...
class Media_gallery {
  ...
  media_init(folder,action,current=false,isRev=false){
    ...
    $.post(siteUrl2+'admin/adp/get_plugins',{
      "action":action,
      "folder":folder
    },function(data){
      // console.log(data);
  ...
  activate_plugins(){
    $.post(siteUrl2+'admin/adp/info_db',{checked : media_gallery.getPackage(),ative_plugins:media_gallery.getPlugins()},function(data){
      console.log(data);
    })
  }
...
$('.bolt').click(()=>{
  $.post(siteUrl2+'admin/adp/init_plugin/DateJS',(data)=>{
    console.log(data);
  })
})
